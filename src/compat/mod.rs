...
pub fn download_steamrt(path: PathBuf, dest: PathBuf, edition: String) -> bool {
    if !path.exists() || edition.is_empty() { false } else {
        ...
        let dl = Downloader::new(url);
        if dl.is_ok() {
            let mut dll = dl.unwrap();
            let p = path.join("steamrt.tar.xz");
            let dld = dll.download(p.as_path(), |_, _| {});
            if dld.is_ok() { extract_archive("".to_string(), p.to_str().unwrap().to_string(), dest.to_str().unwrap().to_string(), false); }
            true
        } else { false }
    }
}