...
use crate::utils::{KuroIndex, KuroResource, extract_archive, move_all, validate_checksum};
...
impl Kuro for Game {
    async fn download<F>(manifest: String, base_url: String, game_path: String, progress: F) -> bool
    where
        F: Fn(u64, u64) + Send + Sync + 'static,
    {
        ...
        if dlr.exists() {
            tokio::fs::remove_dir_all(&dlr).await.unwrap();
        }
        if dlptch.exists() {
            tokio::fs::remove_dir_all(&dlptch).await.unwrap();
        }

        let manifest_file = dlp.clone().join("manifest.json");
        if manifest_file.exists() {
            tokio::fs::remove_file(manifest_file.clone()).await.unwrap();
        }
        ...
        if dll.is_ok() {
            let mut f = tokio::fs::File::open(manifest_file.clone().as_path())
                .await
                .unwrap();
            ...
            if !staging.exists() {
                tokio::fs::create_dir_all(staging.clone()).await.unwrap();
            }
            ...
            for _ in 0..6 {
                let w = Worker::<KuroResource>::new_fifo();
                stealers_list.push(w.stealer());
                workers.push(w);
            }
            let stealers = Arc::new(stealers_list);
            for task in files.resource.into_iter() {
                injector.push(task);
            }
        ...
        F: Fn(u64, u64) + Send + Sync + 'static,
    {
        ...
        if dlp.exists() {
            tokio::fs::remove_dir_all(&dlp).await.unwrap();
        }
            ...
            if preloaded {
                ...
                if files.patch_infos.is_some() {
                    ...
                    for d in diffs {
                        ...
                        if diffp.exists() {
                            tokio::fs::remove_file(diffp).await.unwrap();
                        }
                    }
                }
                ...
                if files.group_infos.is_some() {
                    ...
                    for d in diffs {
                        ...
                        if krd {
                            ...
                            eprintln!("Failed to apply krpdiff!")
                        }
                        if diffp.exists() {
                            tokio::fs::remove_file(diffp).await.unwrap();
                        }
                    ...
                    if files.delete_files.is_some() {
                        ...
                        if !dfl.is_empty() {
                            for df in dfl {
                                ...
                                if dfp.exists() {
                                    tokio::fs::remove_file(&dfp).await.unwrap();
                                }
                            }
                        }
                    }
                        ...
                        loop {
                            ...
                            let ct = tokio::spawn({
                                ...
                                async move {
                                    ...
                                    let pn = chunk_task.dest.clone();
                                    let chunk_base = if chunk_task.dest.ends_with(".krzip")
                                        || chunk_task.dest.ends_with(".krdiff")
                                        || chunk_task.dest.ends_with(".krpdiff")
                                    {
                                        chunk_res
                                    } else {
                                        chunks_zip + "/"
                                    ...
                                    if dlf.is_ok() && cvalid {
                                        ...
                                        if dlf.is_ok() && cvalid {
                                            ...
                                            .unwrap();
                                            let dlf =
                                                dl.download(staging_dir.clone(), |_, _| {}).await;
                                            let cvalid = validate_checksum(
                                                staging_dir.as_path(),
                                                chunk_task.md5.to_ascii_lowercase(),
                                            )
