...
pub fn download_xxmi(dest: &Path) -> Option<bool> {
    if dest.exists() {
        ...
        if rel.is_some() {
            ...
            let response = client.get(dl).header(USER_AGENT, "KeqingLauncher/tauri-app").send().unwrap();
            fs::write(&dest.join("xxmi.zip"), response.bytes().unwrap()).unwrap();
            Some(true)
        } else {
            None
        }
...
pub fn download_xxmi_packages(dest: &Path) -> Option<bool> {
    if dest.exists() {
        ...
        if gimi.is_some() && srmi.is_some() && zzmi.is_some() && wwmi.is_some() {
            ...
            let response0 = client.get(dlg).header(USER_AGENT, "KeqingLauncher/tauri-app").send().unwrap();
            let response1 = client.get(dlsr).header(USER_AGENT, "KeqingLauncher/tauri-app").send().unwrap();
            let response2 = client.get(dlzz).header(USER_AGENT, "KeqingLauncher/tauri-app").send().unwrap();
            let response3 = client.get(dlww).header(USER_AGENT, "KeqingLauncher/tauri-app").send().unwrap();
